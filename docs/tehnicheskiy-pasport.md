...
|---|---|---|
| Разметка | HTML5 | пять страниц: `index.html`, `orders.html`, `stats.html`, `privacy.html`, `custom-order.html` |
...
| Хранение фото | Cloudinary или Supabase Storage | **в планах**; на 18.09.2026 фото по ссылкам из ВК |
...
Схема на 18.09.2026: **17 таблиц, 123 полей, 18 связей**.
...
erDiagram
    ...
    order_statuses ||--o{ status_transitions : "from_status_id"
    order_statuses ||--o{ status_transitions : "to_status_id"
```
...
| `order_statuses` | `status_transitions` | `from_status_id` | запрещено, пока есть ссылки |
| `order_statuses` | `status_transitions` | `to_status_id` | запрещено, пока есть ссылки |
...
| `is_paid` | `boolean` |  | оплачен полностью; это признак, а не статус |
| `paid_at` | `timestamptz` |  | когда отмечен оплаченным |
...
| `changed_role` | `text` |  | роль: пока только admin, в будущем другие |
| `comment` | `text` |  | — |
...
| `pickup_point` | `text` |  | АДРЕС: пункт выдачи или станция метро (не комментарий!) |
| `cost` | `numeric(10,2)` |  | — |
| `shipped_at` | `timestamptz` |  | — |
| `delivered_at` | `timestamptz` |  | — |
| `comment` | `text` |  | свободный комментарий: время, пожелания курьеру |
...
### 4.15. `admin_credentials` — Хеш пароля кабинета заказов

| Поле | Тип | Ключ | Описание |
|---|---|:--:|---|
| `key` | `text` | **`PK`** | единственная строка: admin |
| `password_hash` | `text` | **`NN`** | SHA-256 пароля; сам пароль не хранится |
| `created_at` | `timestamptz` |  | — |
| `updated_at` | `timestamptz` |  | триггер обновляет при смене пароля |

### 4.16. `status_transitions` — Статусная модель (Jira workflow)

| Поле | Тип | Ключ | Описание |
|---|---|:--:|---|
| `from_status_id` | `int` | **`PK, FK`** | откуда разрешён переход |
| `to_status_id` | `int` | **`PK, FK`** | куда разрешён переход |
...
| 10 | **Кабинет заказов** | Прямой адрес `orders.html` → вход по паролю (только с ПК) → список заказов, карточка заказа окном, статусная модель, выгрузка в CSV. |
| 11 | **Статистика заявок** | Вкладка «Статистика» в кабинете или `stats.html` → тот же вход → KPI, заявки по дням, воронка статусов, топ товаров, выгрузка сводки за несколько периодов. |
| 9 | **Документы** | Секция «Документы» после «Условий заказа»: паспорт и брендбук открываются модальными окнами, политика ПДн (`privacy.html`) и руководство по кастомному заказу (`custom-order.html`) — отдельными страницами. Ссылки продублированы в подвале. |
...
### 7.3. Кабинет просмотра заказов и его честные границы

Страница `orders.html` (вход по паролю, только с компьютера) показывает заказы,
справочник статусов и статусную модель в стиле Jira workflow. Данные отдаёт функция
`admin_orders(hash)`: она сравнивает SHA-256 хеш пароля и только тогда возвращает
заказы одним JSON-ом. Таблица `orders` при этом остаётся закрытой для прямого чтения,
а хеш пароля не читается через API: у таблицы `admin_credentials` нет политик RLS.

**Что в этой защите есть:** пароль не хранится и не передаётся в открытом виде;
заказы закрыты политиками RLS; обойти функцию и прочитать `orders` напрямую нельзя;
задать пароль можно только из SQL Editor.

**Чего в ней нет — и это осознанно, проект учебный:**

| Чего нет | Последствие |
|---|---|
| Серверной сессии и токенов | Хеш живёт в sessionStorage вкладки до её закрытия |
| Ограничения частоты попыток на сервере | Подбор пароля перебором теоретически возможен |
| Журнала входов | Не видно, кто и когда открывал кабинет |
| Разделения ролей | Один пароль на всех, кто его знает |
| HTTPS-only cookie и атрибутов настоящей аутентификации | Защита держится только на секретности хеша |

Этот же перечень продублирован в раскрывающемся блоке на самой странице.
Для настоящего магазина раздел нужно перевести на Supabase Auth.

Кабинет заказов (`orders.html`) и статистика (`stats.html`) используют тот же механизм:
хеш пароля сравнивается на сервере, каждая попытка входа пишется в `admin_login_log`
(время, успех, user-agent, действовавший хеш). Обе страницы работают только на просмотр.

### 7.4. Клиентская сторона
...
| Приоритет | Задача |
|---|---|
| Средний | Кабинет просмотра заказов: доработки по итогам использования |
| Высокий | Форма кастомного заказа с записью в `custom_orders` и расчётом предоплаты 40% |
...
| Средний | Разнести `index.html` на отдельные файлы стилей и скриптов |
| Низкий | Перенести фото из ВК в Cloudinary: текущие ссылки работают, задача отложена сознательно |
...
|---|---|---|
| 1.0.0 | 17.09.2026 | Первая редакция |
| 1.1.0 | 18.09.2026 | Уточнены стек, хранение, пользовательские пути |
| 1.2.0 | 18.09.2026 | Схема приведена к фактической; таблицы в табличном виде; темы; документы |
| 1.3.0 | 18.09.2026 | Кабинет просмотра заказов: пароль, статусная модель, CSV |
| 1.4.0 | 18.09.2026 | Промокоды, пункт выдачи, исправление комментария доставки |
| 1.5.0 | 18.09.2026 | Доработки кабинета: пагинация, горизонтальная статусная модель, окно модели |
| 1.6.0 | 18.09.2026 | Убраны дубли полей и ссылок; пункт выдачи скрывается при самовывозе |
| 1.7.0 | 18.09.2026 | Единая трёхкомпонентная версия на всех страницах; новая лицензия; вычитка текстов |
| 1.8.0 | 18.09.2026 | Страница статистики заявок; журнал входов администратора; карточка заказа отдельным окном |
| **1.9.0** | **18.09.2026** | «Оплачен» стал признаком is_paid; смена статусов из кабинета с проверкой переходов; роль в журнале истории |

### Как читается версия

Три компонента по правилам семантического версионирования:
**MAJOR** — несовместимые перемены (другая схема базы или способ публикации);
**MINOR** — новая функциональность (кабинет, промокоды, страницы);
**PATCH** — исправления без новой функциональности.
Номер MINOR совпадает с номером волны работ, чтобы по версии было видно,
какая функциональность уже внутри. Схема приведена к фактической (15 таблиц); добавлены `product_images` и `site_content`; описание таблиц переведено в табличный вид со связями; добавлены разделы о безопасности, `create_order`, управлении контентом и темах оформления; стек очищен от технологий, которых в проекте нет; пользовательские пути приведены в соответствие с работающим сайтом |
