...
-- ----------------------------------------------------------------------------
-- 0. Убираем лишнюю перегрузку функции проверки пароля.
--    Повторный запуск скрипта 06 воссоздавал версию с одним аргументом рядом
--    с версией (text, text), из-за чего вызов с одним аргументом падал с ошибкой
--    «function public.admin_check_password(text) is not unique».
-- ----------------------------------------------------------------------------
